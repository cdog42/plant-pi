...
def write_to_log(port_label, time_plugged_in, time_unplugged, total_time_plugged_in):
    # Check if port_label is in the port_label_mapping
...
class USBDevice:
    def __init__(self, device_name, sys_path, port_label):
        ...
        self.plug_in_time = datetime.now().astimezone()

    def get_time_plugged_in(self):
        return datetime.now().astimezone() - self.plug_in_time
...
def main():
    ...
    for device in iter(monitor.poll, None):
        ...
        if device.action == "add":
            ...
            time_unplugged_str = datetime.now().astimezone().strftime('%Y-%m-%d %H:%M:%S')
            total_time_plugged_in_str = str(usb_device.get_time_plugged_in())
            
            write_to_log(usb_device.port_label, time_plugged_in_str, time_unplugged_str, total_time_plugged_in_str)
